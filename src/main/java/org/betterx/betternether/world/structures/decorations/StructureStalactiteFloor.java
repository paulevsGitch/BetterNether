...
public class StructureStalactiteFloor implements IStructure {
    ...
    private boolean canPlaceAt(ServerLevelAccessor world, BlockPos pos) {
        return world.isEmptyBlock(pos) && (ground == null
                ? BlocksHelper.isNetherGround(world.getBlockState(pos.below()))
                : groundContains(world.getBlockState(pos.below()).getBlock()));
    }
                ...
                if (canPlaceAt(world, POS)) {
                    ...
                    for (int n = 0; n < h; n++) {
                        ...
                        BlocksHelper.setWithoutUpdate(world,
                                                      POS,
                                                      block.defaultBlockState().setValue(BlockStalactite.SIZE, size));
                    }
                }
            }
        }
