...
public class ConfigScreen extends Screen {
	...
	protected void init() {
		...
		header = Component.translatable("\u00A7b* ").append(Component.translatable("config.betternether.mod_reload").getString());
...
//				}).createButton(this.minecraft.options, this.width / 2 - 100, 27, 150);
		...
		AbstractWidget armorButton = new Button(this.width / 2 - 100, 27 * 2, 150, 20, Component.translatable("config.betternether.armour"), new OnPress() {
			@Override
			public void onPress(Button button) {
				...
				button.setMessage(Component.translatable("config.betternether.armour")
						.append(color + CommonComponents.optionStatus(value).getString()));
			}
		});
		String color = hasArmour ? ": \u00A7a" : ": \u00A7c";
		armorButton.setMessage(Component.translatable("config.betternether.armour").append(color + CommonComponents.optionStatus(hasArmour).getString()));
		this.addRenderableWidget(armorButton);

		this.addRenderableWidget(new Button(this.width / 2 + 40 + 20, 27 * 2, 40, 20, Component.translatable("config.betternether.reset"), new OnPress() {
			@Override
			public void onPress(Button button) {
				Configs.MAIN.setBoolean("improvement", varArmour, true);
				BetterNether.setThinArmor(true);
				armorButton.setMessage(Component.translatable("config.betternether.armour").append(": \u00A7a" + CommonComponents.optionStatus(true).getString()));
			}
		}));
		...
		AbstractWidget lavaButton = new Button(this.width / 2 - 100, 27 * 3, 150, 20, Component.translatable("config.betternether.armour"), new OnPress() {
			@Override
			public void onPress(Button button) {
				boolean value = !Configs.MAIN.getBoolean("improvement", varLava, true);
				Configs.MAIN.setBoolean("improvement", varLava, value);
				String color = value ? ": \u00A7a" : ": \u00A7c";
				button.setMessage(Component.translatable("config.betternether.lavafalls")
						.append(color + CommonComponents.optionStatus(value).getString()));
			}
		});
		color = hasLava ? ": \u00A7a" : ": \u00A7c";
		lavaButton.setMessage(Component.translatable("config.betternether.lavafalls").append(color + CommonComponents.optionStatus(hasLava).getString()));
		this.addRenderableWidget(lavaButton);

		this.addRenderableWidget(new Button(this.width / 2 + 40 + 20, 27 * 3, 40, 20, Component.translatable("config.betternether.reset"), new OnPress() {
			@Override
			public void onPress(Button button) {
				Configs.MAIN.setBoolean("improvement", varLava, true);
				BetterNether.setThinArmor(true);
				lavaButton.setMessage(Component.translatable("config.betternether.lavafalls").append(": \u00A7a" + CommonComponents.optionStatus(true).getString()));
			}
		}));
	}
