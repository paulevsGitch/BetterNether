			...
			for (int x = x1; x <= x2; x++) {
				...
				for (int z = z1; z <= z2; z++) {
					...
					if (state.getBlock() == Blocks.NETHER_WART_BLOCK || state.getBlock() == Blocks.WARPED_WART_BLOCK) {
						if (world.isEmptyBlock(belowPos) && world.isEmptyBlock(abovePos) && world.isEmptyBlock(popPos.north()) && world.isEmptyBlock(popPos.south()) && world.isEmptyBlock(popPos.east()) && world.isEmptyBlock(popPos.west()))
							BlocksHelper.setWithoutUpdate(world, popPos, AIR);
						continue;
					}
					
					if (state.getBlock() instanceof BlockStalactite && !(state = world.getBlockState(belowPos)).isCollisionShapeFullBlock(world, belowPos) && !(state.getBlock() instanceof BlockStalactite)) {
