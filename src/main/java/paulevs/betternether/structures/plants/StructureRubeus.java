		...
		for (BlockPos bpos: POINTS)
		{
			if (POINTS.contains(bpos.up()) && POINTS.contains(bpos.down()))
			{
				...
				if (MIDDLE.contains(bpos))
					setCondition(world, bpos, pos.getY(), state.with(BlockRubeusLog.SHAPE, TripleShape.MIDDLE));
				else if (TOP.contains(bpos))
					setCondition(world, bpos, pos.getY(), state.with(BlockRubeusLog.SHAPE, TripleShape.TOP));
				else
					setCondition(world, bpos, pos.getY(), state.with(BlockRubeusLog.SHAPE, TripleShape.BOTTOM));
			}
			else
			{
