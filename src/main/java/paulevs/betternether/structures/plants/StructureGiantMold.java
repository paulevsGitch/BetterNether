...
public class StructureGiantMold implements IStructure {
	...
	public void grow(ServerLevelAccessor world, BlockPos pos, Random random) {
		...
		BlockState middle = BlocksRegistry.GIANT_MOLD.defaultBlockState().setValue(BlockRedLargeMushroom.SHAPE, TripleShape.MIDDLE);
		for (int y = 1; y < size; y++)
			BlocksHelper.setWithoutUpdate(world, pos.above(y), middle);
		BlocksHelper.setWithoutUpdate(world, pos.above(size), BlocksRegistry.GIANT_MOLD.defaultBlockState().setValue(BlockRedLargeMushroom.SHAPE, TripleShape.TOP));
		BlocksHelper.setWithUpdate(world, pos, BlocksRegistry.GIANT_MOLD.defaultBlockState().setValue(BlockRedLargeMushroom.SHAPE, TripleShape.BOTTOM));
	}
}
